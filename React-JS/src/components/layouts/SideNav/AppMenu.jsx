...
// RBAC Filter: Hide Demandes Rôles and Users for non-admin
const getFilteredMenuItems = () => {
  const storedUser = localStorage.getItem('user');
  if (!storedUser) return menuItemsData;

  const user = JSON.parse(storedUser);
  const isAdmin = user.role === 'admin';

  if (isAdmin) return menuItemsData;  // Full for admin

  // Filter Demandes Rôles and Users for non-admin
  return menuItemsData.map(item => {
    if (item.key === 'DemandesRoles' || item.key === 'Users') return null;  // Hide Demandes and Users
    return item;
