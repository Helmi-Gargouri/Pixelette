...
const API_BASE = import.meta.env.VITE_API_URL || 'http://localhost:8000/api';
  ...
  // Nouveau : Fetch des clusters personnels
  const fetchClusters = async () => {
    ...
    try {
      ...
      setError('Erreur lors du chargement des clusters')
    }
  ...
  // Filtrage et tri (pour mode 'list')
  useEffect(() => {
    let result = [...galeries]
  ...
  // Charge les clusters quand mode change à 'clusters'
  useEffect(() => {
  ...
  const generateGaleriePDF = async (galerie) => {
    try {
      ...
      if (galerie.theme) {
        ...
        pdf.setFont('helvetica', 'normal')
        pdf.setTextColor(darkColor[0], darkColor[1], darkColor[2])
        pdf.text(galerie.theme, nextX + 3, infoY + 11)
      }
      
      // ... (le reste de ta fonction generateGaleriePDF reste inchangé, je l'ai truncé pour brevité)

      pdf.save(`${galerie.nom}_export.pdf`)
    } catch (err) {
    ...
    <>
      ...
      <div className="space">
        <div className="container">
            ...
            <div className="row align-items-center g-2">
              ...
              <div className="col-lg-2 col-md-3 col-6">
                ...
                >
                  ...
                  <option value="oeuvres">Nb œuvres</option>
                </select>
              </div>
              <div className="col-lg-2 col-md-3 col-6">
                <select
                ...
                >
                  ...
                  <option value="clusters">Clusters IA</option>
                </select>
              </div>
              <div className="col-lg-1 col-md-12">
                <Link to="/galeries/create" className="btn w-100" style={{ padding: '0 10px', height: '44px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px' }}>
                  <i className="fas fa-plus me-1"></i>Créer
                </Link>
              </div>
            </div>
          </div>

          {/* État vide pour mode 'list' */}
          {viewMode === 'list' && filteredGaleries.length === 0 && (
          ...
          {/* Rendu pour mode 'list' */}
          {viewMode === 'list' && filteredGaleries.length > 0 && filteredGaleries.map((galerie) => {
          ...
          {viewMode === 'clusters' && (
            <>
              {clustersLoading ? (
                ...
                <div className="text-center py-5">
                  <i className="fas fa-images fa-4x text-muted mb-4"></i>
                  <h3 className="text-muted mb-3">Aucun cluster disponible</h3>
                  <p className="text-muted mb-4">Pas assez de galeries avec descriptions pour clusteriser.</p>
                </div>
              ) : (
                Object.entries(clusters).map(([label, galeriesInCluster]) => (
                  <div key={label} style={{ marginBottom: '60px' }}>
                    <h2 className="mb-4">{label}</h2>
                    {galeriesInCluster.map((galerie) => {
                      const currentSlideIndex = currentSlides[galerie.id] || 0
                      const oeuvresList = galerie.oeuvres_list || galerie.oeuvres || []  // Adapte si 'oeuvres_list' n'est pas dans le serializer
                      const hasOeuvres = oeuvresList.length > 0
                      const currentOeuvre = hasOeuvres ? oeuvresList[currentSlideIndex] : null
                      ...
                      return (
                        <div key={galerie.id} style={{ marginBottom: '60px' }}>
                          ...
                          <div className="hero-wrapper hero-3" style={{ marginBottom: '0', paddingBottom: '40px' }}>
                            <div className="container">
                              <div className="hero-style3">
                                <h1 className="hero-title">{galerie.nom}</h1>
                                <h1 className="hero-title style2">{galerie.theme || 'Collection d\'Art'}</h1>
                                <div className="hero-thumb3-1">
                                  {currentOeuvre?.image ? (
                                    <img
                                      src={currentOeuvre.image}
                                    ...
                                    <img src="/assets/img/hero/hero_3_1.png" alt="Galerie d'art" />
                                  )}
                                  <Link to={`/galeries/${galerie.id}`} className="btn gsap-magnetic">
                                    Explorer
                                  </Link>
                                  <div className="event-vanue-details">
                                    <span>Créée le {new Date(galerie.date_creation).toLocaleDateString('fr-FR')}</span>
                          ...
                          {/* Boutons d'action pour la galerie (même en mode clusters) */}
                          <div className="text-center mt-3 mb-4">
                            <Link to={`/galeries/${galerie.id}/oeuvres`} className="btn btn-sm me-2">
                              <i className="fas fa-cog me-1"></i> Gérer les œuvres
                            </Link>
                            <Link
                              to={`/galeries/${galerie.id}/edit`}
                              className="btn btn-sm me-2"
                            ...
                            <button
                              ...
                              style={{ backgroundColor: '#28a745', borderColor: '#28a745', color: '#fff' }}
                            >
                              <i className="fas fa-file-pdf me-1"></i> Exporter PDF
                            </button>
                            <button
                              onClick={() => handleDeleteClick(galerie.id)}
                              className="btn btn-sm"
